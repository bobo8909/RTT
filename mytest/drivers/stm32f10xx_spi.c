                                ...
                                rt_size_t size)
{
    ...
    DMA_DeInit(stm32_spi_bus->DMA_Channel_RX);
    DMA_DeInit(stm32_spi_bus->DMA_Channel_TX);
    ...
    /* Check if the DMA Stream is disabled before enabling it.
       ...
       be bypassed. */
    while (DMA_GetCmdStatus(stm32_spi_bus->DMA_Channel_RX) != DISABLE);
    while (DMA_GetCmdStatus(stm32_spi_bus->DMA_Channel_TX) != DISABLE);
